...
namespace Server.Message
{
    public class VesselMsgReader : ReaderBase
    {
        public override void HandleMessage(ClientStructure client, IClientMessageBase message)
        {
            ...
            if (messageData != null)
                LunaLog.Debug($"Vessel msg type: {messageData.VesselMessageType} from {client.PlayerName} vessel {messageData.VesselId}");
            switch (messageData?.VesselMessageType)
            {
                ...
                case VesselMessageType.Position:
                    if (messageData is VesselPositionMsgData posMsg && !VesselStoreSystem.CurrentVessels.ContainsKey(posMsg.VesselId))
                        VesselDataUpdater.CreateVesselFromPositionIfNeeded(posMsg.VesselId, posMsg);
        ...
        private static void HandleVesselRemove(ClientStructure client, VesselBaseMsgData message)
        {
            ...
            if (data.AddToKillList && VesselStoreSystem.VesselExists(data.VesselId))
            {
